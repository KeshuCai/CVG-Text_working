...
if __name__ == "__main__":
    ...
    if rank == 0:
        ...
        print("Text File:", [trainset_path, testset_path])
    ...
    for epoch in range(num_epochs):
        ...
        if rank == 0:
            ...
            print(f"Epoch {epoch+1}/{num_epochs}: Loss: {avg_loss:.4f}, R@1: {r_at_1:.2f}%, R@5: {r_at_5:.2f}%, R@10: {r_at_10:.2f}%, ")#t: {model.module.logit_scale.item():.4f}")
        ...
        if (epoch + 1) % 1 == 0:
            if rank == 0:
                ...
                if r_at_1_test > best_test_r_at_1:
                    best_test_r_at_1 = r_at_1_test
                    best_epoch = epoch

                    if log:
                        os.makedirs(os.path.join(checkpoint_dir, safe_model_name), exist_ok=True)
                        checkpoint_path = os.path.join(checkpoint_dir, safe_model_name, f'{hyperparams}_best.pt')
                        torch.save(model.module.state_dict(), checkpoint_path)
                        print(f"New best model saved with R@1: {best_test_r_at_1:.2f}%")
    if rank == 0 and log:
